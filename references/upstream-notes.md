...
| 7a | Codex has no `whenToUse` field in frontmatter | v0.2.0: removed `whenToUse` from all four skills after confirming that DSH's `@deepseek-ai/dsh-tool-skill@0.1.0-rc.6` renders only `name` + `description` in the model-facing catalog; `whenToUse` is validated and stored by the registry but never injected into the model-visible `<available_skills>` list or the `<skill_content>` block. Routing triggers now live entirely in the `description` field. | |
